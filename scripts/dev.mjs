...
const handle=createApi({query:async(s,p)=>(await db.query(s,p)).rows,preview:true,adminToken:process.env.ADMIN_SETUP_TOKEN});
...
createServer(async(req,res)=>{
 try{
  ...
  if(url.pathname.startsWith('/api/')){
   const chunks=[];let length=0;for await(const part of req){length+=part.length;if(length>20000){res.writeHead(413);res.end();return;}chunks.push(part);}
   const request=new Request(url,{method:req.method,headers:req.headers,...(req.method==='POST'?{body:Buffer.concat(chunks)}:{})});
   const response=await handle(request,{ip:req.socket.remoteAddress});res.writeHead(response.status,Object.fromEntries(response.headers));res.end(await response.text());return;
  }
