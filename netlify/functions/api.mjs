...
import { createApi, storedPhotoKey, storedPhotoResponse } from '../../server/api.mjs';
import { verifiedIdentityFromRequest, identityPasswordLogin } from '../../server/identity.mjs';
let handle;
export default async (request,context)=>{
 try{
  // Do not make public image loading depend on Identity or a database connection.
  const photoKey=storedPhotoKey(request);
  if(photoKey){const store=getStore({name:'product-photos',consistency:'strong'});return await storedPhotoResponse(photoKey,{get:key=>store.get(key,{type:'arrayBuffer'})});}
