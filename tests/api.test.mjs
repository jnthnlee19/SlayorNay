...
function client(){let identityUser=null;return {async use(id){identityUser=await seedIdentity(db,id);return this;},async call(path,body,extra={}){const response=await handle(new Request('https://example.test/api'+path,{headers:{origin:'https://example.test','content-type':'application/json',...extra},...(body!==undefined?{method:'POST',body:JSON.stringify(body)}:{})}),{ip:'127.0.0.1',identityUser,identityPasswordLogin:async()=>{const e=new Error('Wrong password');e.status=401;throw e;}});return {status:response.status,data:await response.json(),headers:response.headers};}};}
...
test('verified email profiles are used for saved votes',async()=>{await alice.use('alice');await bob.use('bob');assert.equal((await alice.call('/me')).data.user.email,'alice@example.test');});
...
test('duplicate and concurrent votes count once; saved vote belongs to account',async()=>{
 ...
 const otherDevice=client();await otherDevice.use('alice');assert.equal((await otherDevice.call('/products')).data.products.find(p=>p.id==='opi-bubble-bath').my_vote,'slay');
});
...
test('email sign-in failures are rate limited',async()=>{let last;for(let i=0;i<35;i++)last=await anon.call('/email-signin',{email:'missing@example.test',password:'wrong123'});assert.equal(last.status,429);});

test.after(async()=>db.close());
test('admin can add a custom category and edit a product without replacing its votes',async()=>{
 await db.query("DELETE FROM rate_limits"); const owner=client(),voter=client();await owner.use('categoryowner');await voter.use('categoryvoter');await db.query("UPDATE users SET is_admin=true WHERE username='categoryowner'");
...
test('deletion is admin-only and removes product votes; category moves preserve data',async()=>{
 await db.query('DELETE FROM rate_limits');const owner=client(),voter=client();await owner.use('deleteowner');await voter.use('deletevoter');await db.query("UPDATE users SET is_admin=true WHERE username='deleteowner'");
...
test('vote changes replace one owned vote, including concurrent retries',async()=>{
 const user=client(),stranger=client();await user.use('vote_editor');await stranger.use('vote_stranger');
